...
import { LogOut, Sun, Moon, Bell, X, User } from 'lucide-react';
...
export default function Navbar() {
  ...
  const dashboardLink = role === "ADMIN" ? "/admin-dashboard" : "/dashboard"; // ⭐ Added
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-indigo-800 text-white shadow-lg border-b border-indigo-700">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        {/* ⭐ LOGO redirects to correct dashboard */}
        <Link
        ...
        {/* ⭐ DASHBOARD LINK (role protected) */}
        <div className="hidden md:flex gap-8 text-sm font-medium">
          {isLoggedIn && (
            <Link
              to={dashboardLink}
              className={`${isActive(dashboardLink) ? "text-brand-purple" : "hover:text-[rgb(var(--fg))]"} transition-colors`}
              style={{ color: "rgb(var(--fg))" }}
            >
              Dashboard
            </Link>
          ...
          */}
          ...
          {isLoggedIn ? (
            <>
              ...
              <Link
                to="/profile"
                className="w-9 h-9 rounded-full bg-indigo-500 hover:bg-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-md transition-all border-2 border-indigo-400/30"
                title="View Profile"
              >
